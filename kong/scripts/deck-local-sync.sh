...
if [ ! -f "${KONG_YAML}" ]; then
  echo "ERROR: kong.yaml not found. Run Terraform first:"
  echo "  cd ${PROJECT_DIR} && terraform apply"
  exit 1
fi

# Validate YAML structure offline
echo "==> Validating kong.yaml (offline)..."
command deck file validate "${KONG_YAML}"
echo "    OK"

if [ "${DIFF_ONLY}" = true ]; then
  echo ""
  echo "==> Dumping current Kong state for comparison..."
  command deck gateway dump --kong-addr "${KONG_ADMIN_URL}" -o /tmp/kong-current.yaml 2>/dev/null || true
  echo "    Current state saved to /tmp/kong-current.yaml"
  echo "    Compare with: diff /tmp/kong-current.yaml ${KONG_YAML}"
  exit 0
fi

echo ""
echo "==> This will sync Kong configuration with kong.yaml (diff & apply)."
...
echo ""
echo "==> Syncing kong.yaml (diff & apply)..."
command deck gateway sync "${KONG_YAML}" --kong-addr "${KONG_ADMIN_URL}"
echo ""
